...
1. 下载最新版本的插件 JAR 文件（`plugin-codeenhance-2.0.0.jar`）
2. 登录 Halo 管理后台
3. 进入「插件管理」→「安装插件」
4. 选择下载的 JAR 文件上传安装
5. 安装完成后启用插件
...
# 构建产物位于 build/libs/plugin-codeenhance-2.0.0.jar
```
...
插件使用 Halo 2.x 的扩展点机制，通过 `HeadProcessor` 向页面注入前端资源：

```java
@Bean
public HeadProcessor codeEnhanceHeadProcessor() {
    return context -> {
        // 根据配置注入前端资源
        if (config.isEnableCodeFold() || config.isEnableImgFold()) {
            // 注入 code-enhance.js 和 code-enhance.css
        }
    };
}
```
